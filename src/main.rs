...
async fn handle_get(
    ...
    let permit = match state.encode_limiter.clone().acquire_owned().await {
        Ok(permit) => permit,
        Err(_) => return response_with_status(StatusCode::SERVICE_UNAVAILABLE, "service unavailable"),
    };
    ...
    if let Err(err) = write_cache_file(&cache_entry, &converted).await {
        warn!(
            path = %uri,
...
async fn fetch_upstream(
    ...
    request.send().await.context("failed to send upstream request")
}
...
async fn reqwest_response_to_axum(response: reqwest::Response, streaming: bool) -> Response<Body> {
    ...
    if streaming {
        ...
        match response.bytes().await {
            Ok(bytes) => builder.body(Body::from(bytes)).expect("bytes response is valid"),
            Err(err) => {
...
async fn write_cache_file(entry: &CacheEntry, bytes: &[u8]) -> anyhow::Result<()> {
    let parent = entry
        .final_path
        .parent()
    ...
    let mut file = fs::File::create(&entry.temp_path).await?;
    file.write_all(bytes).await?;
    file.flush().await?;
    drop(file);

    match fs::rename(&entry.temp_path, &entry.final_path).await {
        Ok(()) => Ok(()),
        Err(err) => {
...
impl Config {
    fn from_env() -> anyhow::Result<Self> {
        ...
        let cache_dir = PathBuf::from(
            env::var("CACHE_DIR").unwrap_or_else(|_| DEFAULT_CACHE_DIR.to_string()),
        );
        ...
        let default_encodes = std::thread::available_parallelism()
            ...
            .unwrap_or(1);
        let max_concurrent_encodes =
            parse_env_usize("MAX_CONCURRENT_ENCODES", default_encodes)?;
...
fn parse_env_u64(name: &str, default: u64) -> anyhow::Result<u64> {
    env::var(name)
        .map(|value| value.parse::<u64>().with_context(|| format!("{name} must be u64")))
        .unwrap_or(Ok(default))
}

fn parse_env_usize(name: &str, default: usize) -> anyhow::Result<usize> {
    let value = env::var(name)
        .map(|value| value.parse::<usize>().with_context(|| format!("{name} must be usize")))
        .unwrap_or(Ok(default))?;
    if value == 0 {
...
fn parse_env_u8(name: &str, default: u8) -> anyhow::Result<u8> {
    env::var(name)
        .map(|value| value.parse::<u8>().with_context(|| format!("{name} must be u8")))
        .unwrap_or(Ok(default))
}

fn parse_env_f32(name: &str, default: f32) -> anyhow::Result<f32> {
    env::var(name)
        .map(|value| value.parse::<f32>().with_context(|| format!("{name} must be number")))
        .unwrap_or(Ok(default))
}
...
mod tests {
    ...
    fn accept_negotiation_can_be_enabled() {
        let uri: Uri = "/bucket/a.jpg".parse().unwrap();
        let mut headers = HeaderMap::new();
        headers.insert(ACCEPT, HeaderValue::from_static("image/avif;q=0,image/webp"));
    ...
    fn bypass_path_strips_format_before_proxying() {
        ...
        assert_eq!(url.as_str(), "http://127.0.0.1:9100/my-bucket/a%20b.jpg?v=1");
    }
}
